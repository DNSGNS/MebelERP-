...
public partial class LayoutEditorContentView : ContentView
{
    ...
    public LayoutEditorContentView()
    {
        InitializeComponent();
    }
    ...
    private async void OnRemoveSheetClicked(object sender, EventArgs e)
    {
        ...
        bool answer = await Application.Current.MainPage.DisplayAlert(
            "Удаление стола",
            "Вы уверены? Детали с этого стола переместятся на склад.",
            "Удалить",
            "Отмена");
        ...
        if (sheet.Parts.Count > 0)
        {
            ...
            else if (sheetColor != partColor)
            {
                await Application.Current.MainPage.DisplayAlert("Ошибка",
                    $"Нельзя поместить деталь цвета '{partColor}' на стол с цветом '{sheetColor}'.", "OK");
                return;
            }
        }
        // --------------------------------
